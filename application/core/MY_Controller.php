...
class MY_Controller extends CI_Controller {
    ...
    public function render_logged_in($body, $data=array()){
        ...
        $data['main_content'] = $this->load->view($this->_unit . '/' . $body,$data,true);
		...
		if(isset($this->session->userdata('login')->role_name) === true){
			switch ($this->session->userdata('login')->role_name) {
				case 'Administrator':
					...
					break;
				default:
                    die('You have no permission to access the page');
					break;
			}
		}
	}
...
class MY_Controller_Admin extends MY_Controller {
    ...
    public function __construct() {
        ...
        if(isset($this->user->role_name) === false){
            redirect('login');
        }
        ...
        if(isset($this->user->id) === false && $this->user->role_name != 'Administrator'){
            redirect('login');
        }

        if($this->user->role_name !== 'Administrator'){
            $this->redirect();
        }
    ...
    public function render($body, $data = array()){
        $this->css_header .= css(array('bootstrap-4.0.0-alpha.5-dist/css/dataTables.bootstrap4.min','bootstrap-4.0.0-alpha.5-dist/css/glyphicons'));
        $this->js_footer .= javascript(array(
            'bootstrap-4.0.0-alpha.5-dist/js/jquery.dataTables.min',
            'bootstrap-4.0.0-alpha.5-dist/js/dataTables.bootstrap.min',
            'users/admin/common'
        ));

        $this->render_logged_in($this->_role.'/'.$body,$data);
    }
}
